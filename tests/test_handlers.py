...
class TestBuiltinFormatHandlerCsvDialect:
    ...
    def test_read_uses_dialect_delimiter(self, handler):
        ...
        stream = io.BytesIO(b"a;b\n1;2\n3;4\n")
        df = handler.read(stream, format="csv", dialect=CsvDialect(delimiter=";")).payload
        assert list(df.columns) == ["a", "b"]
        assert len(df) == 2
    ...
    def test_read_dialect_header_false_skips_header_row(self, handler):
        ...
        df = handler.read(stream, format="csv", dialect=CsvDialect(header=False)).payload
    ...
    def test_read_explicit_kwargs_win_over_dialect(self, handler):
        ...
        stream = io.BytesIO(b"a,b\n1,2\n")
        df = handler.read(stream, format="csv", sep=",", dialect=CsvDialect(delimiter=";")).payload
        assert list(df.columns) == ["a", "b"]
    ...
    def test_read_dialect_is_ignored_for_non_csv(self, handler):
        ...
        stream = io.BytesIO(b'[{"a": 1, "b": 2}]')
        df = handler.read(stream, format="json", dialect=CsvDialect(delimiter=";")).payload
        assert list(df.columns) == ["a", "b"]

    def test_write_uses_dialect_delimiter(self, handler):
        from sunstone.asset import Asset, AssetKind
        from sunstone.lineage import CsvDialect, Metadata

        stream = io.BytesIO()
        df = pd.DataFrame({"a": [1, 2], "b": [3, 4]})
        asset = Asset(payload=df, kind=AssetKind.TABULAR, metadata=Metadata())
        handler.write(asset, stream, format="csv", index=False, dialect=CsvDialect(delimiter=";"))
        stream.seek(0)
    ...
    def test_write_dialect_header_false_omits_header(self, handler):
        ...
        df = pd.DataFrame({"a": [1, 2], "b": [3, 4]})
        asset = Asset(payload=df, kind=AssetKind.TABULAR, metadata=Metadata())
        handler.write(asset, stream, format="csv", index=False, dialect=CsvDialect(header=False))
        stream.seek(0)
    ...
    def test_dialect_none_is_no_op(self, handler):
        ...
        stream = io.BytesIO(b"a,b\n1,2\n")
        df = handler.read(stream, format="csv", dialect=None).payload
        assert list(df.columns) == ["a", "b"]
