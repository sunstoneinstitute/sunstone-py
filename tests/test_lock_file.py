...
class TestLockFileWriting:
    ...
    def test_lock_file_header_not_duplicated_across_writes(self, lock_project: Path) -> None:
        """Repeated saves must not stack the auto-generated header comment."""
        source = DatasetMetadata(name="Input Data", slug="input-data", location="inputs/data.csv")
        lineage = LineageMetadata(sources=[source])
        ...
        for i in range(3):
            ...
            manager.update_output_lineage(
                slug="output-data",
                lineage=lineage,
                data_hash=f"sha256:cycle-{i}",
            )

        text = (lock_project / "datasets.lock.yaml").read_text()
        header_count = text.count("# Auto-generated by sunstone. Do not edit manually.")
        assert header_count == 1, f"Header appeared {header_count} times:\n{text}"

    def test_lock_file_compacts_existing_duplicate_headers(self, lock_project: Path) -> None:
        """Files written by the old (buggy) version should be cleaned up on next save."""
        lock_path = lock_project / "datasets.lock.yaml"
        original = lock_path.read_text()
        # Simulate a checked-in file produced by the buggy version: 5 stacked headers
        lock_path.write_text("# Auto-generated by sunstone. Do not edit manually.\n" * 5 + original)
        assert lock_path.read_text().count("# Auto-generated by sunstone.") == 5

        manager = DatasetsManager(lock_project)
        manager.update_output_lineage(
            slug="output-data",
            lineage=LineageMetadata(
                sources=[DatasetMetadata(name="Input Data", slug="input-data", location="inputs/data.csv")]
            ),
            data_hash="sha256:after-migration",
        )

        text = lock_path.read_text()
        header_count = text.count("# Auto-generated by sunstone. Do not edit manually.")
        assert header_count == 1, f"Header appeared {header_count} times after migration:\n{text}"
