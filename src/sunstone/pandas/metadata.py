"""
MetadataMixin — property accessors and field-metadata helpers for the
Sunstone DataFrame.

This mixin is consumed by `sunstone.pandas.core.DataFrame` via multiple
inheritance. The properties below proxy through ``self.metadata`` (a
``Metadata`` container provided by the concrete subclass) to expose
convenience accessors; ``set_field_metadata`` writes through to the
unified ``field_metadata`` mapping and, when ``source`` is provided,
maintains a parallel ``FieldDerivation`` entry on the lineage.

Note: pandas is not needed here — this mixin operates purely on the
metadata container — so we keep imports minimal.
"""
...
class MetadataMixin:
    ...
    - ``metadata`` — a :class:`~sunstone.lineage.Metadata` container.

    The five property pairs (``lineage``, ``description``, ``rdf_prefixes``,
    ``custom_properties``, ``unit_display``) proxy through ``self.metadata``
    (with ``unit_display`` falling back to an instance attribute). The
    ``set_field_metadata`` method writes column-level metadata into
    ``self.metadata.field_metadata`` and maintains a matching
    ``FieldDerivation`` on the lineage when ``source`` is supplied.

    The expectation is encoded as a ``TYPE_CHECKING`` stub below so mypy
    can verify usage inside the mixin without runtime overhead.
    """

    if TYPE_CHECKING:
        # Attribute provided by the concrete subclass. ``metadata`` is a
        # ``@property`` on the concrete subclass, so we declare it as a
        # property here too — a bare attribute stub would trigger pyright's
        # "overrides symbol of same name" diagnostic.
        @property
    ...
    def set_field_metadata(
        ...
        *,
        ...
        # Return the concrete subclass instance for method chaining; cast via
        # Any to keep the public type as DataFrame without importing it at
        # runtime (would cycle through sunstone.pandas.core).
